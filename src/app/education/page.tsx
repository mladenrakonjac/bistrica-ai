...
export const metadata: Metadata = {
  ...
  description: "Learn the foundations of Agentic AI, LLMs, and AI pipelines — focused on insurance applications and European regulatory context.",
};
...
const concepts = [
  ...
  { term: "Human-in-the-Loop (HITL)", definition: "A design pattern where the AI agent escalates specific decisions to a human reviewer — typically based on confidence thresholds, case complexity, or policy requirements." },
  ...
  { term: "Tool use / Function calling", definition: "The ability of an LLM to invoke external tools — APIs, databases, calculators, search — enabling agents to act on the real world, not just generate text." },
  { term: "Data drift", definition: "The phenomenon where the distribution of data an agent encounters in production differs from training/development data — causing degraded performance over time." },
];
  ...
  return (
    <>
      ...
      <section className="relative pt-32 pb-16 bg-off-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-deep-navy/60 text-lg max-w-2xl leading-relaxed">
            Practical education for insurance professionals and technology teams — from foundational concepts to production architecture decisions.
          </p>
        </div>
      </section>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-xl font-bold text-deep-navy">Articles & Guides</h2>
            <span className="text-deep-navy/40 text-sm">{articles.length} articles</span>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {articles.map((article) => (
              <Card key={article.title} className="border-border hover:border-royal-blue/30 hover:shadow-md transition-all duration-200 group cursor-pointer hover:-translate-y-0.5">
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <Badge variant="secondary" className="bg-sky-blue/10 text-royal-blue border-sky-blue/20 text-xs">
                      {article.category}
                    </Badge>
                    <span className="text-deep-navy/35 text-xs">{article.readTime} read</span>
                  </div>
                </CardHeader>
                <CardContent>
                  <h3 className="text-deep-navy font-bold text-base mb-2 leading-snug group-hover:text-royal-blue transition-colors">
                    {article.title}
                  </h3>
                  <p className="text-deep-navy/60 text-sm leading-relaxed mb-4">{article.description}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {article.tags.map((tag) => (
                      <Badge key={tag} variant="outline" className="border-border text-deep-navy/40 text-xs px-2 py-0.5">
                        {tag}
                      </Badge>
                    ))}
                  </div>
      ...
      <section className="py-16 bg-off-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-10">
            <p className="text-royal-blue text-sm font-semibold uppercase tracking-widest mb-3">Glossary</p>
            <h2 className="text-3xl font-black text-deep-navy">Key concepts explained</h2>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            {concepts.map((c) => (
              <Card key={c.term} className="border-border bg-white">
                <CardContent className="p-5">
                  <h3 className="text-deep-navy font-bold text-sm mb-1.5">{c.term}</h3>
                  <p className="text-deep-navy/60 text-sm leading-relaxed">{c.definition}</p>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-3 gap-6">
            {[
              ...
              <Card key={profile.role} className="border-border">
                <CardContent className="p-6">
                  <h3 className="text-deep-navy font-bold text-lg mb-2">{profile.role}</h3>
                  <p className="text-deep-navy/60 text-sm leading-relaxed mb-4">{profile.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {profile.topics.map((topic) => (
                      <Badge key={topic} variant="secondary" className="bg-sky-blue/10 text-royal-blue border-sky-blue/20 text-xs">
                        {topic}
                      </Badge>
                    ))}
                  </div>
