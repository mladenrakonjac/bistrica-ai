...
export const metadata: Metadata = {
  ...
  description: "Discover how Agentic AI transforms insurance operations — claim processing, OCR, fraud detection, underwriting, and more.",
};
...
const useCases = [
  ...
  },
  {
    ...
    description: "Policies, damage reports, medical records, invoices, ID documents, certificates of coverage — insurance generates massive amounts of unstructured data. Our document intelligence pipeline uses vision-language models to extract structured data from any format, in any European language, with confidence scoring and validation against business rules.",
    outcomes: [
  ...
  },
  {
    ...
    description: "Underwriting decisions require synthesising data from many sources — credit, property, health, weather, claims history. AI pipelines automate data enrichment, compute risk scores, generate policy comparison summaries, and surface recommendations to underwriters — dramatically reducing the time from application to quote while improving consistency.",
    outcomes: [
    ...
    <>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          {useCases.map((uc, i) => (
            <Card key={uc.id} className="border-border hover:border-royal-blue/30 hover:shadow-md transition-all duration-300">
              <CardContent className="p-8 lg:p-10">
                <div className="grid lg:grid-cols-2 gap-10">
                  <div>
                    <div className="flex items-center gap-3 mb-4">
                      <Badge variant="secondary" className="bg-sky-blue/10 text-royal-blue border-sky-blue/20">
                        {uc.tag}
                      </Badge>
                      <span className="text-deep-navy/30 text-sm font-medium">0{i + 1}</span>
                    </div>
                    <h2 className="text-2xl sm:text-3xl font-black text-deep-navy mb-2">{uc.title}</h2>
                    <p className="text-royal-blue font-medium mb-5">{uc.headline}</p>
                    <p className="text-deep-navy/60 leading-relaxed text-sm sm:text-base">{uc.description}</p>
                  </div>

                  <div className="space-y-6">
                    <div>
                      <h3 className="text-deep-navy font-semibold text-sm mb-3">Outcomes</h3>
                      <ul className="space-y-2">
                        {uc.outcomes.map((outcome) => (
                          <li key={outcome} className="flex items-start gap-3 text-deep-navy/70 text-sm">
                    ...
                    <div>
                      <h3 className="text-deep-navy font-semibold text-sm mb-3">Technology Components</h3>
                      <div className="flex flex-wrap gap-2">
                        {uc.stack.map((item) => (
                          <Badge key={item} variant="outline" className="border-border text-deep-navy/60 text-xs">
                            {item}
                          </Badge>
                        ))}
                      </div>
